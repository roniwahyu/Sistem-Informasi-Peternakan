...
class kartustok extends MX_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('kartustok_model','kartustokdb',TRUE);
    ...
    public function get($id=null){
        if($id!==null){
            echo json_encode($this->kartustokdb->get_one($id));
        }
    }
    function tables(){
    ...
    function getone($id=null){
        if($id!==null){
            $data=$this->kartustokdb->get_one($id);
            $jml=count($data);
            // print_r($jml);
            // print_r($data);
            $div='';
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('id')){
            $this->kartustokdb->update($this->input->post('id'));
          }else{
            $this->kartustokdb->save();
          }
    ...
    public function delete(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->kartustokdb->delete($this->input->post('id'));
     ...
     public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->kartustokdb->delete_detail($this->input->post('id'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
    ...
    private function gen_faktur(){
        $last=$this->kartustokdb->get_last_pt();
        // print_r($last);
        if(!empty($last)):

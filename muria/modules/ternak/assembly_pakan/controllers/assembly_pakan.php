...
class assembly_pakan extends MX_Controller {
    ...
    public function index() {
        $this->template->set_title('Kelola Pencampuran Stok (Pakan)');
        $this->template->add_js('var baseurl="'.base_url().'assembly_pakan/";','embed');  
        $this->template->load_view('assembly_pakan_view',array(
            'view'=>'',
            'title'=>'Kelola Data Pencampuran Stok (Pakan)',
            'subtitle'=>'Pengelolaan Pencampuran Stok (Pakan)',
            'breadcrumb'=>array(
            'Assembly_pakan'),
        ));
    }
    ...
    public function getdatatables(){
        if($this->isadmin()==1):
            $this->datatables->select('id,faktur,tanggal,id_gudang,id_recording,id_formulasi,akun_perkiraan,total,is_trx,is_void,is_jrnl,is_post,date_posted,id_user,datetime,')
                            ->from('assembly_pakan');

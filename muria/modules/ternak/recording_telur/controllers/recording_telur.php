...
class recording_telur extends MX_Controller {
    ...
    public function index() {
     ...
     public function baru($type) {
        ...
        $gudang=$this->telurdb->dropdown_gudang();
    ...
    public function submit(){
        ...
        $detail=$this->telurdb->get_detail($faktur);
        $kode=substr($faktur_ref,2,1);
        foreach ($detail as $k => $val) {
            $datastok[] = array(
                    ...
                    'tipe' => 'D',
                    'jumlah' =>  $val['jumlah_total'],
                    'id_barang' => $val['id_barang'],
                    'id_satuan' => $val['id_satuan'],
                    'debet' =>  $val['jumlah_total'],
                    'kredit' =>0,
            ...
            if($kode=="P"){
                $datastok[]["tipe"]="K";
                $datastok[]["debet"]=0;
                $datastok[]["kredit"]=$this->input->post('jumlah_total', TRUE);
                $datastok[]["keterangan"]="Pemakaian Telur: ".$tgl." - ".$ket;
            }
        }
              ...
              if ($id){
               // $this->telurdb->update_detail($id);
                ...
                $this->telurdb->save_detail($data);
                  if($kode=="S"):
                    // $this->submit_stok($id);
                    $this->stokdb->save_stok($datastok);
                endif;
              }
          }
        }

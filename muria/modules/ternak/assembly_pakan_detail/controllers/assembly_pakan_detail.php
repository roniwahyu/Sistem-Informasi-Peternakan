...
class assembly_pakan_detail extends MX_Controller {

    function __construct() {
        parent::__construct();
          
        //Load IgnitedDatatables Library
        $this->load->model('assembly_pakan_detail_model','assembly_pakan_detaildb',TRUE);
        $this->session->set_userdata('lihat','assembly_pakan_detail');
    ...
    public function index() {
        $this->template->set_title('Kelola Assembly_pakan_detail');
        $this->template->add_js('var baseurl="'.base_url().'assembly_pakan_detail/";','embed');  
        $this->template->load_view('assembly_pakan_detail_view',array(
            'view'=>'',
            'title'=>'Kelola Data Assembly_pakan_detail',
            'subtitle'=>'Pengelolaan Assembly_pakan_detail',
            'breadcrumb'=>array(
            'Assembly_pakan_detail'),
        ));
    }
    ...
    function enkrip(){
        ...
        // echo $this->session->userdata('assembly_pakan_detail');
    }
    function isadmin(){
    ...
    function forms(){
        ...
        $this->load->view('assembly_pakan_detail_form_inside');
           
    }

    public function get($id_detail=null){
        if($id_detail!==null){
            echo json_encode($this->assembly_pakan_detaildb->get_one($id_detail));
        }
    }
    function tables(){
        $this->load->view('assembly_pakan_detail_data');
    }

    function getone($id=null){
        if($id!==null){
            $data=$this->assembly_pakan_detaildb->get_one($id);
            $jml=count($data);
            // print_r($jml);
            // print_r($data);
            $div='';
    ...
    public function submit(){
        if ($this->input->post('ajax')){
          if ($this->input->post('id_detail')){
            $this->assembly_pakan_detaildb->update($this->input->post('id_detail'));
          }else{
            $this->assembly_pakan_detaildb->save();
          }
    ...
    public function delete(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id'])){
                $this->assembly_pakan_detaildb->delete($this->input->post('id'));
     ...
     public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id_detail'])){
                $this->assembly_pakan_detaildb->delete_detail($this->input->post('id_detail'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
    ...
    private function gen_faktur(){
        $last=$this->assembly_pakan_detaildb->get_last_pt();
        // print_r($last);
        if(!empty($last)):
...
/** Module assembly_pakan_detail Controller **/

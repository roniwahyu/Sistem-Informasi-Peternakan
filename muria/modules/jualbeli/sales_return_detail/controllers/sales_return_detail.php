...
class sales_return_detail extends MX_Controller {
    ...
    function enkrip(){
        ...
        // echo $this->session->userdata('sales_return_detail');
    }
    function isadmin(){
     ...
     public function delete_detail(){
        if(isset($_POST['ajax'])){
            if(!empty($_POST['id_detail'])){
                $this->sales_return_detaildb->delete_detail($this->input->post('id_detail'));
                $this->session->set_flashdata('notif','Succeed, Data Has Deleted');
            }else {
                $this->session->set_flashdata('notif', 'Failed! No Data Deleted');
            }
    ...
    private function gen_faktur(){
        $last=$this->sales_return_detaildb->get_last_pt();
        // print_r($last);
        if(!empty($last)):
